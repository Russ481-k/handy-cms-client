...
export function ScheduleList({
  ...
  const handleAddMenu = (parentMenu: Menu) => {
    console.log(
      "ScheduleList handleAddMenu called with parentMenu:",
      parentMenu
    );

    // 전체 일정인 경우 (id가 -1)
    if (parentMenu.id === -1) {
      // 전체 일정를 부모로 사용하고 parentId를 undefined로 설정
      onAddMenu({
        ...parentMenu,
        parentId: undefined,
      });
  ...
  const handleMoveMenu = (
    ...
    // 전체 일정로의 이동은 최상위로 이동하는 것으로 처리
    if (targetId === -1) {
      onMoveMenu(draggedId, 0, position); // 0은 최상위 일정를 나타냄
      return;
    }
  ...
  // forceExpandMenuId가 변경될 때 해당 일정를 강제로 펼치고 포커스
  useEffect(() => {
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      ...
      <ConfirmDialog
        ...
        title="일정 삭제"
        description={`"${menuToDelete?.name}" 일정를 삭제하시겠습니까?`}
        confirmText="삭제"
        cancelText="취소"
